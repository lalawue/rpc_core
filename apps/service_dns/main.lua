...
local Log = require("middle.logger").newLogger("[DNS]", "error")
...
local function _initUdpQueryInterface()
   ...
   for i = 1, 2 do
      ...
      udp_chann:setCallback(function(_, event_name, _, c_msg)
...
local function _sprotoServiceHandler(self, proto_info, reqeust_object, rpc_response)
   if proto_info and
      proto_info.name == AppEnv.Service.DNS_SPROTO.name and
      type(reqeust_object.domain) == "string"
   then
      _updateWaitingList(self, reqeust_object.domain, rpc_response)
      return true
   else
...
function App:loadBusiness( rpc_framework )
   ...
   rpc_framework.newService(AppEnv.Service.DNS_JSON, function(a, b, c)
      return _httpJsonServiceHandler(self, a, b, c)
   end)

   -- init Sproto service RPC
   rpc_framework.newService(AppEnv.Service.DNS_SPROTO, function(a, b, c)
