...
local HttpJsonParser = {
    m_hp = nil, -- hyperparser
    m_left_data = nil    
}
HttpJsonParser.__index = HttpJsonParser

function HttpJsonParser.newRequest()
    local parser = setmetatable({}, HttpJsonParser)
    parser.m_hp = HttpParser.createParser("REQUEST")
    return parser
end

function HttpJsonParser.newResponse()
    local parser = setmetatable({}, HttpJsonParser)
...
function HttpJsonParser:process(data)
...
function HttpJsonParser:destroy()
    if self.m_hp then
        self.m_hp:destroy()
        self.m_hp = nil
    end
